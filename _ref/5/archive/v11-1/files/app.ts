...
function checkNearHint(shape: ClonedShape): boolean {
  ...
  for (let i = 0; i < entry.shapes.length; i++) {
    ...
    // If angle is also correct, it means shape is ready to snap
    // If angle is wrong, the glow hints user needs to rotate
    if (dist < nearThreshold) {
      ...
      if (!angleMatch) {
        // Position close but angle wrong - show near hint border as a rotation hint
        return true;
      }
      // Both position and angle are close - also show hint (snap will happen on release)
      return true;
    }
  }
...
function trySnapToTarget(shape: ClonedShape): boolean {
  ...
  for (let i = 0; i < entry.shapes.length; i++) {
    ...
    // Strict angle check: user MUST have rotated shape to near-correct angle BEFORE snap
    ...
    // Both position AND angle must be within threshold - angle is NOT auto-corrected
  ...
  if (bestIndex >= 0) {
    // Snap animation - ONLY moves position (x, y). Angle is NEVER changed.
    gsap.to(shape, {
      x: bestTargetX,
      y: bestTargetY,
      duration: 0.18,
  ...
  // If distance is close but angle is wrong, show near hint effect only
  for (let i = 0; i < entry.shapes.length; i++) {
    const expectedType = entry.shapes[i].type as ShapeType;
    if (shape.type !== expectedType) continue;
    ...
    if (dist < appData.snapThreshold) {
      // Position is close but angle is wrong - show near hint glow
      shape.nearHint = true;
      break;
    }
  }
