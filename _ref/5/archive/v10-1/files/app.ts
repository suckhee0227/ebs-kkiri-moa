...
// Function to check angle match with symmetry awareness
function isAngleMatchWithSymmetry(
  ...
  if (shapeType === ShapeType.RECTANGLE) {
    const mod = Math.PI / 2;
    let diff = (currentRotation - targetRotation) % mod;
    if (diff < 0) diff += mod;
  ...
  const diff = Math.abs(normalizeAngle(currentRotation - targetRotation));
  return diff < thresholdRad;
}

// Function to find nearest symmetric-equivalent target rotation for snapping
function findNearestSymmetricRotation(shapeType: ShapeType, currentRotation: number, targetRotation: number): number {
  if (shapeType === ShapeType.CIRCLE) {
  ...
  if (shapeType === ShapeType.RECTANGLE) {
    ...
    symmetryPeriod = (Math.PI * 2) / 3;
  } else {
    return targetRotation;
  }

  // Find the nearest rotation equivalent to targetRotation
  let best = targetRotation;
  let bestDiff = Math.abs(normalizeAngle(currentRotation - targetRotation));

  for (let k = -4; k <= 4; k++) {
    const candidate = targetRotation + k * symmetryPeriod;
    const diff = Math.abs(normalizeAngle(currentRotation - candidate));
    if (diff < bestDiff) {
      bestDiff = diff;
      best = candidate;
    }
  ...
  for (const entry of answerMap) {
    ...
    for (let i = 0; i < entry.shapes.length; i++) {
      ...
      const matchingShape = clonedShapes.find((s) => {
        ...
        return isAngleMatchWithSymmetry(s.type, s.rotation, targetRotation, appData.snapAngleThreshold * 2);
      });
...
// Function to check near-hint proximity for a shape (visual hint only, no snapping or movement)
function checkNearHint(shape: ClonedShape): boolean {
  ...
  const nearAngleThreshold = appData.nearHintAngleThreshold || 35;
  ...
  for (let i = 0; i < entry.shapes.length; i++) {
    ...
    const angleMatch = isAngleMatchWithSymmetry(shape.type, shape.rotation, targetRotation, nearAngleThreshold);

    // [FIX 2] Near hint is PURELY visual - only shows glow border, never moves/rotates/snaps the shape
    if (dist < nearThreshold && angleMatch) {
      return true;
    }
  }
...
// Function to try snapping a shape to a target position within a drop zone (only on mouseup)
function trySnapToTarget(shape: ClonedShape): boolean {
  ...
  for (let i = 0; i < entry.shapes.length; i++) {
    ...
    // [FIX 3] Strict angle check: user MUST have rotated shape to near-correct angle
    const angleMatch = isAngleMatchWithSymmetry(shape.type, shape.rotation, targetRotation, appData.snapAngleThreshold);
    ...
    // [FIX 3] Both position AND angle must be within threshold - no exceptions
  ...
  if (bestIndex >= 0) {
    // Find nearest symmetric rotation for smooth snap animation
    const snapRotation = findNearestSymmetricRotation(shape.type, shape.rotation, bestTargetRotation);

    // Snap animation - only triggers when user already had correct position AND angle
    gsap.to(shape, {
      x: bestTargetX,
      y: bestTargetY,
...
function handlePointerMove(e: PointerEvent): void {
  ...
  if (isRotating && selectedShape) {
    ...
    let newRotation = shapeStartAngle + deltaAngle;

    // Apply rotation directly - only rotation handle changes angle
  ...
  if (draggingShape) {
    // [FIX 1] Only update position during drag - NEVER change angle
    draggingShape.x = x - dragOffsetX;
    draggingShape.y = y - dragOffsetY;
    updateShapeDropZone(draggingShape);

    // [FIX 2] During drag, only update visual hint (nearHint) - NO snapping, NO position forcing, NO rotation
...
function handlePointerUp(e: PointerEvent): void {
  ...
  if (isRotating) {
    ...
    // [FIX 3] On rotation end, try snap only if position AND angle both match
  ...
  if (draggingShape) {
    ...
    if (draggingShape.y > appData.paletteY - appData.shapeSize * 0.5 && draggingShape.dropZoneIndex === -1) {
      ...
      // [FIX 2] Snap judgment ONLY happens here at mouseup - checks both position AND angle
