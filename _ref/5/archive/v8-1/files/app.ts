...
function checkAllAnswers(): boolean {
  ...
  for (const entry of answerMap) {
    ...
    // Check each expected shape in this drop zone
    for (let i = 0; i < entry.shapes.length; i++) {
      ...
      // Find a matching snapped shape at this position
      const matchingShape = clonedShapes.find((s) => {
        ...
        const angleDiff = Math.abs(normalizeAngle(s.rotation - targetRotation));
        const posThreshold = appData.snapThreshold * 1.5;
        const angleThreshold = (appData.snapAngleThreshold * 2 * Math.PI) / 180;

        return dist < posThreshold && angleDiff < angleThreshold;
      });
...
function drawTargetIntersection(target: ITargetIntersection): void {
  ...
  // Check if both required shapes are snapped into this drop zone - if so, hide the target silhouette
  const snappedInZone = clonedShapes.filter((s) => s.dropZoneIndex === dz.index && s.snapped);
  if (snappedInZone.length >= 2) return;

  // Also hide target if there are 2+ shapes in the drop zone (even if not snapped, show live blend instead)
...
function initTargetIntersections(): void {
  // Build target intersections from answerMap data for perfect consistency
  targetIntersections = [];

  // Color mixing map: blend two palette colors to get the intersection color
  // Blue (#5B8DEF) + Yellow (#FFD966) = Green
  // Yellow (#FFD966) + Red (#FF6B6B) = Orange
  // Blue (#5B8DEF) + Red (#FF6B6B) = Purple
  const colorMixMap: { [key: string]: string } = {
  ...
  for (const entry of appData.answerMap) {
    const color1 = entry.shapes[0].color.toUpperCase();
    const color2 = entry.shapes[1].color.toUpperCase();
...
function drawClonedShapes(): void {
  ...
  shapesByDropZone.forEach((shapes, dzIndex) => {
    if (shapes.length >= 2) {
      const dz = dropZones[dzIndex];
      if (!dz) return;
      // For each pair, draw the intersection with blended color
...
function trySnapToTarget(shape: ClonedShape): boolean {
  ...
  for (let i = 0; i < entry.shapes.length; i++) {
    ...
    if (dist < appData.snapThreshold && angleDiff < angleThresholdRad && dist < bestDist) {
      bestDist = dist;
      bestIndex = i;
      bestTargetX = targetX;
      bestTargetY = targetY;
  ...
  if (bestIndex >= 0) {
    // Animate snap for smooth feel
    gsap.to(shape, {
      x: bestTargetX,
      y: bestTargetY,
      rotation: bestTargetRotation,
      duration: 0.15,
      ease: "power2.out",
    });
    shape.snapped = true;
...
function handlePointerMove(e: PointerEvent): void {
  ...
  if (isRotating && selectedShape) {
    ...
    let newRotation = shapeStartAngle + (angle - rotationStartAngle);

    // Snap-assist: if close to a target snap angle, snap to it
    const snapAngleThresholdRad = (appData.snapAngleThreshold * Math.PI) / 180;
    const snapAssistThresholdRad = (15 * Math.PI) / 180; // 15 degrees proximity for snap assist

    // Build list of snap target angles for the current shape in its drop zone
    const snapAngles: number[] = [];
    // Always include common angles: 0, 90, 180, 270
    snapAngles.push(0, Math.PI / 2, Math.PI, -Math.PI / 2);

    // Add answer-map target angles for this shape type and color in its drop zone
    if (selectedShape.dropZoneIndex >= 0) {
      const entry = appData.answerMap.find((e) => e.dropZoneIndex === selectedShape!.dropZoneIndex);
      if (entry) {
        for (let i = 0; i < entry.shapes.length; i++) {
          if (
            entry.shapes[i].type === selectedShape.type &&
            entry.shapes[i].color.toUpperCase() === selectedShape.color.toUpperCase()
          ) {
            const targetAngle = (entry.targetOffsets[i].rotation * Math.PI) / 180;
            snapAngles.push(targetAngle);
          }
        }
      }
    }

    // Check proximity to snap angles
    let snappedAngle = newRotation;
    let closestDist = Infinity;
    for (const sa of snapAngles) {
      const diff = Math.abs(normalizeAngle(newRotation - sa));
      if (diff < snapAssistThresholdRad && diff < closestDist) {
        closestDist = diff;
        snappedAngle = sa;
      }
    }

    // Apply snap with a magnetic pull effect (interpolate when very close)
    if (closestDist < snapAssistThresholdRad && closestDist !== Infinity) {
      const pullStrength = 1.0 - closestDist / snapAssistThresholdRad;
      selectedShape.rotation = newRotation + (snappedAngle - newRotation) * pullStrength * pullStrength;
    } else {
...
function handlePointerUp(e: PointerEvent): void {
  ...
  if (draggingShape) {
    ...
    if (draggingShape.y > appData.paletteY - appData.shapeSize * 0.5 && draggingShape.dropZoneIndex === -1) {
      ...
      // Try snapping to target position
...
// Function to create the playing state UI elements (check answer and reset buttons)
...
function handleCheckAnswer(): void {
  ...
  if (isCorrect) {
    ...
    // Shake unsnapped shapes to indicate wrong answer
