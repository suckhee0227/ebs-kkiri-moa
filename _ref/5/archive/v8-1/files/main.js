...
function handlePointerMove(e) {
  ...
  if (isRotating && selectedShape) {
    ...
    const snapAssistThresholdRad = 15 * Math.PI / 180;
    const snapAngles = [];
    snapAngles.push(0, Math.PI / 2, Math.PI, -Math.PI / 2);
    if (selectedShape.dropZoneIndex >= 0) {
      const entry = appData.answerMap.find((e2) => e2.dropZoneIndex === selectedShape.dropZoneIndex);
      if (entry) {
        for (let i = 0; i < entry.shapes.length; i++) {
          if (entry.shapes[i].type === selectedShape.type && entry.shapes[i].color.toUpperCase() === selectedShape.color.toUpperCase()) {
    ...
    if (closestDist < snapAssistThresholdRad && closestDist !== Infinity) {
      const pullStrength = 1 - closestDist / snapAssistThresholdRad;
