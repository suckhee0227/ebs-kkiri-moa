...
// public/20260401183100_f1bbc072-ea26-42d7-a4eb-8de16f4caed3/appHelper.ts
var AppHelper;
...
// public/20260401183100_f1bbc072-ea26-42d7-a4eb-8de16f4caed3/app.ts
function getEquilateralTriangleVertices(cx, cy, size, rotation) {
...
function anglesMatchWithSymmetry(angle1, angle2, type, thresholdDeg) {
...
function drawShape(targetCtx, type, x, y, size, color, rotation, alpha, glowing) {
  targetCtx.save();
  ...
  if (glowing) {
    const pulse = 0.6 + 0.4 * Math.sin(glowPulseTime * 8e-3);
...
function isPointInShape(px2, py, shape) {
  const dx = px2 - shape.x;
  const dy = py - shape.y;
...
function isPointOnRotationHandle(px2, py, shape) {
...
function updateShapeGlowState(shape) {
  if (shape.snapped) {
    shape.glowing = false;
    return;
  }
...
function drawGuideCharacter() {
  ...
  ctx.fillText("\u{1F9D1}\u200D\u{1F33E}", charX, charY);
  ctx.restore();
...
function drawInstructionScreen() {
  ...
  drawShape(ctx, "rectangle" /* RECTANGLE */, appData.canvasWidth * 0.78, 180, 50, "#5B8DEF", 0.15, 0.75, false);
  drawShape(ctx, "triangle" /* TRIANGLE */, appData.canvasWidth * 0.85, 265, 50, "#FFD966", 0.3, 0.75, false);
  drawShape(ctx, "circle" /* CIRCLE */, appData.canvasWidth * 0.8, 345, 50, "#FF6B6B", 0, 0.75, false);
...
function findSymmetricSnapAngle(currentAngle, targetAngle, type) {
  if (type === "circle" /* CIRCLE */) {
    return currentAngle;
  }
  let symmetryPeriod;
...
function showSnapCheckmark(x, y) {
...
function drawCheckmarkAnimations() {
...
function render3() {
  ...
  ctx.globalAlpha = 1;
  ctx.globalCompositeOperation = "source-over";
  animFrameId = requestAnimationFrame(render3);
}
...
function handlePointerDown(e) {
  ...
  if (currentState === "title" /* TITLE */) {
    ...
    currentState = "instruction" /* INSTRUCTION */;
    return;
  }
  if (currentState === "instruction" /* INSTRUCTION */) {
...
function showCorrectOverlay() {
  const overlay = AppHelper.createUIElement(
    ...
    "\u{1F389} \uC815\uB2F5\uC785\uB2C8\uB2E4! \u{1F389}",
    []
  );
  uiLayer.appendChild(overlay);
  uiElements.push(overlay);
  gsapWithCSS.to(overlay.style, {
    opacity: "1",
    duration: 0.5,
    ease: "power2.out"
  });
...
var targetIntersections, assetListData, nextCloneId, currentState, appData, textData, appCanvas, ctx, uiLayer, clonedShapes, paletteShapes, dropZones, selectedShape, draggingShape, isDraggingFromPalette, isRotating, dragOffsetX, dragOffsetY, rotationStartAngle, shapeStartAngle, currentBubbleText, animFrameId, uiElements, soundsInitialized, glowPulseTime, checkmarkAnimations;
var init_app = __esm({
  "public/20260401183100_f1bbc072-ea26-42d7-a4eb-8de16f4caed3/app.ts"() {
    init_appHelper();
    init_gsap();
...
// public/20260401183100_f1bbc072-ea26-42d7-a4eb-8de16f4caed3/main.ts
