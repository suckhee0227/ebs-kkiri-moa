...
interface IAppData {
  ...
  hintGlowColor: string;
  hintProximityThreshold: number;
  hintAngleProximityThreshold: number;
}
...
// Glow animation timestamp for pulsating neon effect
let glowPulseTime = 0;

// Temporary checkmark positions for animation
let checkmarkAnimations: { x: number; y: number; alpha: number; scale: number }[] = [];

// Function to compute the equilateral triangle vertices given center and size
function getEquilateralTriangleVertices(cx: number, cy: number, size: number, rotation: number): number[][] {
  // Equilateral triangle: all sides equal, inscribed in circle of radius = size/√3 from centroid
  // Using side length = size, the circumradius R = size / √3
...
function drawShapeIntersectionBlend(shapeA: ClonedShape, shapeB: ClonedShape, dz: DropZone): void {
  ...
  // Fill shape B area (only visible in A∩B region due to clip)
  ctx.beginPath();
...
// Function to check if two angles match considering shape symmetry
function anglesMatchWithSymmetry(angle1: number, angle2: number, type: ShapeType, thresholdDeg: number): boolean {
  if (type === ShapeType.CIRCLE) {
    return true; // Circle matches any rotation
  }

  const thresholdRad = (thresholdDeg * Math.PI) / 180;

  let symmetryPeriod: number;
  if (type === ShapeType.RECTANGLE) {
    symmetryPeriod = Math.PI / 2; // 90 degrees
  } else if (type === ShapeType.TRIANGLE) {
    symmetryPeriod = (2 * Math.PI) / 3; // 120 degrees
  } else {
    symmetryPeriod = Math.PI * 2;
  }

  // Compute angle difference modulo symmetry period
  let diff = (angle1 - angle2) % symmetryPeriod;
  if (diff < 0) diff += symmetryPeriod;
  if (diff > symmetryPeriod / 2) diff = symmetryPeriod - diff;

  return Math.abs(diff) < thresholdRad;
}

// Function to check if all drop zones have the correct shapes snapped in correct positions
  ...
  for (const entry of answerMap) {
    ...
    for (let i = 0; i < entry.shapes.length; i++) {
      ...
      const matchingShape = clonedShapes.find((s) => {
        ...
        return anglesMatchWithSymmetry(s.rotation, targetRotation, expectedType, appData.snapAngleThreshold * 2);
      });
...
function buildShapePath(
  ...
  switch (type) {
    case ShapeType.RECTANGLE: {
      // Perfect square: width = height = size
      const half = size / 2;
    ...
    case ShapeType.TRIANGLE: {
      ...
      const verts = getEquilateralTriangleVertices(cx, cy, size, rotation);
      targetCtx.moveTo(verts[0][0], verts[0][1]);
      for (let i = 1; i < verts.length; i++) {
        targetCtx.lineTo(verts[i][0], verts[i][1]);
      }
      targetCtx.closePath();
      break;
    }
    case ShapeType.CIRCLE: {
      // Perfect circle: radius = size/2
...
function drawTargetIntersection(target: ITargetIntersection): void {
  ...
  // Fill shape B within clip of shape A → only intersection visible
  ctx.beginPath();
...
function drawShape(
  ...
  switch (type) {
    case ShapeType.RECTANGLE: {
      ...
      targetCtx.fillRect(-half, -half, size, size);
      targetCtx.shadowBlur = 0;
      targetCtx.shadowOffsetY = 0;
      targetCtx.strokeStyle = "rgba(255,255,255,0.4)";
      targetCtx.lineWidth = 2;
      targetCtx.strokeRect(-half + 1, -half + 1, size - 2, size - 2);
      targetCtx.strokeStyle = "rgba(0,0,0,0.08)";
      targetCtx.lineWidth = 1;
      targetCtx.strokeRect(-half, -half, size, size);
      break;
    }
    case ShapeType.TRIANGLE: {
      // Perfect equilateral triangle centered at origin
  ...
  // Draw neon glow if shape is glowing (proximity hint)
  if (glowing) {
    const pulse = 0.6 + 0.4 * Math.sin(glowPulseTime * 0.008);
    targetCtx.shadowColor = appData.hintGlowColor || "#ADFF2F";
    targetCtx.shadowBlur = 18 * pulse;
    targetCtx.shadowOffsetY = 0;
    targetCtx.strokeStyle = appData.hintGlowColor || "#ADFF2F";
    targetCtx.lineWidth = 3;
    targetCtx.globalAlpha = 0.7 + 0.3 * pulse;
...
function isPointInShape(
  ...
  switch (shape.type) {
    ...
    case ShapeType.TRIANGLE: {
      ...
      const R = shape.size / Math.sqrt(3);
      const verts: number[][] = [];
      for (let i = 0; i < 3; i++) {
        const angle = (i * 2 * Math.PI) / 3 - Math.PI / 2;
        verts.push([R * Math.cos(angle), R * Math.sin(angle)]);
      }
      const d1 = (lx - verts[2][0]) * (verts[0][1] - verts[2][1]) - (verts[0][0] - verts[2][0]) * (ly - verts[2][1]);
      const d2 = (lx - verts[0][0]) * (verts[1][1] - verts[0][1]) - (verts[1][0] - verts[0][0]) * (ly - verts[0][1]);
      const d3 = (lx - verts[1][0]) * (verts[2][1] - verts[1][1]) - (verts[2][0] - verts[1][0]) * (ly - verts[1][1]);
...
function drawRotationHandle(targetCtx: CanvasRenderingContext2D, shape: ClonedShape): void {
  ...
  switch (shape.type) {
    case ShapeType.RECTANGLE: {
      const half = shape.size / 2 + padding;
      targetCtx.strokeRect(-half, -half, half * 2, half * 2);
      break;
    }
    case ShapeType.TRIANGLE: {
      const R = shape.size / Math.sqrt(3) + padding;
      targetCtx.beginPath();
      for (let i = 0; i < 3; i++) {
...
// Function to initialize palette shapes with perfect geometric proportions
function initPaletteShapes(): void {
  ...
  // Blue perfect square (1:1 aspect ratio enforced by using same size for width and height)
  paletteShapes.push({
  ...
  // Yellow perfect equilateral triangle (all sides equal, all angles 60°)
  paletteShapes.push({
  ...
  // Red perfect circle (not ellipse)
  paletteShapes.push({
...
function drawPaletteShapes(): void {
  for (const ps of paletteShapes) {
    ...
    ctx.globalAlpha = 1.0;
    ctx.globalCompositeOperation = "source-over";
    drawShape(ctx, ps.type, ps.x, ps.y, ps.size, ps.color, 0, 0.9, false);
    ctx.restore();
  }
}

// Function to check proximity of a shape to any target and update glow state
function updateShapeGlowState(shape: ClonedShape): void {
  if (shape.snapped) {
    shape.glowing = false;
    return;
  }
  ...
  const proximityThreshold = appData.hintProximityThreshold || 80;
  const angleProximityThreshold = appData.hintAngleProximityThreshold || 35;
  ...
  for (let i = 0; i < entry.shapes.length; i++) {
    ...
    const angleMatch = anglesMatchWithSymmetry(shape.rotation, targetRotation, shape.type, angleProximityThreshold);

    if (dist < proximityThreshold && angleMatch) {
      shape.glowing = true;
      return;
    }
  }
...
function drawClonedShapes(): void {
  // Group shapes by drop zone for intersection rendering
  const shapesByDropZone: Map<number, ClonedShape[]> = new Map();

  for (const s of clonedShapes) {
    // Update glow state for proximity hint
    updateShapeGlowState(s);
  ...
  // Draw all cloned shapes with independent colors
  for (let i = 0; i < clonedShapes.length; i++) {
    ...
    if (isInDropZone) {
      ...
      drawRoundedRect(ctx, dz.x, dz.y, dz.width, dz.height, appData.dropZoneCornerRadius);
      ctx.clip();
      drawShape(ctx, s.type, s.x, s.y, s.size, s.color, s.rotation, 0.85, s.glowing);
      ctx.restore();
    } else {
      ctx.save();
      ctx.globalCompositeOperation = "source-over";
  ...
  // Draw blended intersection areas for drop zones with 2+ shapes
    ...
    if (shapes.length >= 2) {
      ...
      for (let a = 0; a < shapes.length; a++) {
        for (let b = a + 1; b < shapes.length; b++) {
          drawShapeIntersectionBlend(shapes[a], shapes[b], dz);
        }
      }
    }
  });
...
// Function to draw the guide character with speech bubble (explorer emoji)
function drawGuideCharacter(): void {
  const charX = 60;
  const charY = appData.paletteY - appData.shapeSize * 0.8 - 45;

  // Draw character (explorer emoji)
  ctx.save();
  ctx.globalAlpha = 1.0;
  ctx.globalCompositeOperation = "source-over";
  ...
  // Draw speech bubble to the right
...
function drawTitleScreen(): void {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    ctx.globalCompositeOperation = "source-over";
    drawShape(ctx, types[i % 3], sx, sy, size, colors[i % 3], rot, 0.12, false);
    ctx.restore();
  }
  ctx.restore();
...
function drawInstructionScreen(): void {
  ...
  drawShape(ctx, ShapeType.RECTANGLE, appData.canvasWidth * 0.78, 180, 50, "#5B8DEF", 0.15, 0.75, false);
  drawShape(ctx, ShapeType.TRIANGLE, appData.canvasWidth * 0.85, 265, 50, "#FFD966", 0.3, 0.75, false);
  drawShape(ctx, ShapeType.CIRCLE, appData.canvasWidth * 0.8, 345, 50, "#FF6B6B", 0, 0.75, false);
...
// Function to find the best snap target angle considering shape symmetry
function findSymmetricSnapAngle(currentAngle: number, targetAngle: number, type: ShapeType): number {
  if (type === ShapeType.CIRCLE) {
    return currentAngle; // Circle doesn't need rotation snap
  }

  let symmetryPeriod: number;
  if (type === ShapeType.RECTANGLE) {
    symmetryPeriod = Math.PI / 2; // 90°
  } else if (type === ShapeType.TRIANGLE) {
    symmetryPeriod = (2 * Math.PI) / 3; // 120°
  } else {
    symmetryPeriod = Math.PI * 2;
  }

  // Find the closest equivalent angle to currentAngle that matches targetAngle modulo symmetryPeriod
  let bestAngle = targetAngle;
  let bestDist = Infinity;

  // Try different multiples of symmetryPeriod
  for (let k = -4; k <= 4; k++) {
    const candidate = targetAngle + k * symmetryPeriod;
    const dist = Math.abs(normalizeAngle(currentAngle - candidate));
    if (dist < bestDist) {
      bestDist = dist;
      bestAngle = candidate;
    }
...
function trySnapToTarget(shape: ClonedShape): boolean {
  ...
  for (let i = 0; i < entry.shapes.length; i++) {
    ...
    // Use symmetry-aware angle check
    const angleMatch = anglesMatchWithSymmetry(
      shape.rotation,
      targetRotation,
      expectedType,
    ...
    if (dist < appData.snapThreshold && angleMatch && dist < bestDist) {
      bestDist = dist;
      bestIndex = i;
      bestTargetX = targetX;
      bestTargetY = targetY;
      bestTargetRotation = targetRotation;
    }
  }

  if (bestIndex >= 0) {
    // Find the closest symmetric snap angle
    const snapAngle = findSymmetricSnapAngle(shape.rotation, bestTargetRotation, shape.type);
    ...
    gsap.to(shape, {
      ...
      ease: "back.out(1.5)",
      onComplete: () => {
        // Flash the shape briefly
        setBubbleText(textData.snapBubble);
        setTimeout(() => {
    ...
    // Show a checkmark animation on the snapped shape
    showSnapCheckmark(bestTargetX, bestTargetY);
...
// Function to show a snap checkmark animation at a position
function showSnapCheckmark(x: number, y: number): void {
  const anim = { x, y, alpha: 1, scale: 0.5 };
  checkmarkAnimations.push(anim);
  gsap.to(anim, {
    scale: 1.5,
    alpha: 0,
    duration: 0.8,
    ease: "power2.out",
    onComplete: () => {
      const idx = checkmarkAnimations.indexOf(anim);
      if (idx >= 0) checkmarkAnimations.splice(idx, 1);
    },
  });
}

// Function to draw checkmark animations
function drawCheckmarkAnimations(): void {
  for (const anim of checkmarkAnimations) {
    ctx.save();
    ctx.globalAlpha = anim.alpha;
    ctx.globalCompositeOperation = "source-over";
    ctx.translate(anim.x, anim.y - 30 * anim.scale);
    ctx.font = `${28 * anim.scale}px sans-serif`;
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText("✅", 0, 0);
    ctx.restore();
...
function drawPlayingScreen(): void {
  ...
  const missionX = 36; // Left aligned
  ...
  // White text left-aligned inside capsule
  ctx.fillStyle = "#FFFFFF";
  ctx.font = "bold 14px sans-serif";
  ctx.textAlign = "left";
  ctx.textBaseline = "middle";
...
function handlePointerMove(e: PointerEvent): void {
  ...
  if (isRotating && selectedShape) {
    ...
    const newRotation = shapeStartAngle + (angle - rotationStartAngle);

    // For circles, don't actually rotate (visual is same)
    if (selectedShape.type === ShapeType.CIRCLE) {
  ...
  if (draggingShape) {
    ...
    // Update cursor based on whether near a rotation handle
    if (selectedShape && !selectedShape.snapped && isPointOnRotationHandle(x, y, selectedShape)) {
  ...
  if (selectedShape && !selectedShape.snapped && isPointOnRotationHandle(x, y, selectedShape)) {
    appCanvas.style.cursor = "grab";
  } else if (findClonedShapeAt(x, y) || findPaletteShapeAt(x, y)) {
...
function handlePointerUp(e: PointerEvent): void {
  ...
  if (draggingShape) {
    ...
    if (draggingShape.y > appData.paletteY - appData.shapeSize * 0.5 && draggingShape.dropZoneIndex === -1) {
      ...
      // Try snapping to target position (auto-snap)
  ...
  const btnCheck = AppHelper.createUIElement(
    ...
    [
      ...
      },
      {
        event: "pointerenter",
        handler: () => {
          btnCheck.style.transform = "scale(1.05)";
          btnCheck.style.boxShadow = "0 6px 20px rgba(66, 133, 244, 0.5)";
        },
      },
      {
        event: "pointerleave",
        handler: () => {
          ...
          btnCheck.style.boxShadow = "0 4px 14px rgba(66, 133, 244, 0.35)";
        },
      },
    ],
  );
  ...
  if (isCorrect) {
    ...
    try {
      confetti({
        ...
        spread: 80,
        origin: { y: 0.6 },
        colors: ["#5B8DEF", "#FFD966", "#FF6B6B", "#4CAF50", "#FF9832", "#8E44C7"],
      });
    } catch (_e) {
      // confetti may not work in all environments
    }

    // Animate correct answer celebration
    for (const s of clonedShapes) {
      if (s.snapped) {
        gsap.to(s, {
          scale: 1.15,
...
// Function to show a correct answer overlay animation
function showCorrectOverlay(): void {
  ...
  gsap.to(overlay.style, {
    ...
    onComplete: () => {
      ...
      const idx = uiElements.indexOf(overlay);
      if (idx >= 0) uiElements.splice(idx, 1);
    },
  });
}
