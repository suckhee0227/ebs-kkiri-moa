...
// Function to check near-hint proximity for a shape being dragged
...
function trySnapToTarget(shape: ClonedShape): boolean {
  ...
  if (bestIndex >= 0) {
    // Find nearest symmetric rotation for smooth snap
    ...
    // Check if all shapes in this drop zone are now snapped → auto check
    setTimeout(() => {
      checkAutoComplete();
    }, 250);
...
function render(): void {
  ...
  // Update near hint status for dragging/selected shapes
  for (const s of clonedShapes) {
...
function handlePointerMove(e: PointerEvent): void {
  ...
  if (isRotating && selectedShape) {
    ...
    // Smooth rotation - apply directly
