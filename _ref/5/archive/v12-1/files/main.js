...
function isAngleMatchWithSymmetry(shapeType, currentRotation, targetRotation, thresholdDeg) {
  ...
  const diffDeg = currentDeg - targetDeg;
  if (shapeType === "rectangle" /* RECTANGLE */) {
    let remainder = diffDeg % 90;
    remainder = (remainder % 90 + 90) % 90;
    if (remainder <= thresholdDeg || remainder >= 90 - thresholdDeg) {
      return true;
    }
    return false;
  }
  if (shapeType === "triangle" /* TRIANGLE */) {
    let remainder = diffDeg % 120;
    remainder = (remainder % 120 + 120) % 120;
    if (remainder <= thresholdDeg || remainder >= 120 - thresholdDeg) {
      return true;
    }
    return false;
  }
