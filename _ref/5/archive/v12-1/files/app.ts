...
function isAngleMatchWithSymmetry(
  ...
  // Convert both angles to degrees for clearer modular arithmetic
  ...
  if (shapeType === ShapeType.RECTANGLE) {
    // Rectangle has 90-degree rotational symmetry
    // Check if (diffDeg % 90) is near 0
    let remainder = diffDeg % 90;
    // Normalize remainder to [0, 90)
    remainder = ((remainder % 90) + 90) % 90;
    // remainder is now in [0, 90)
    // It's a match if remainder is close to 0 or close to 90
  ...
  if (shapeType === ShapeType.TRIANGLE) {
    // Equilateral triangle has 120-degree rotational symmetry
    // Check if (diffDeg % 120) is near 0
    let remainder = diffDeg % 120;
    // Normalize remainder to [0, 120)
    remainder = ((remainder % 120) + 120) % 120;
    // remainder is now in [0, 120)
    // It's a match if remainder is close to 0 or close to 120
  ...
  // Fallback for other shapes: direct angle comparison
