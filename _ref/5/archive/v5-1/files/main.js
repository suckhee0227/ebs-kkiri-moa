...
function initTargetIntersections() {
  targetIntersections = [
    {
      dropZoneIndex: 0,
      color: "rgba(34, 139, 94, 0.65)",
      opacity: 1,
      shapes: [
        { type: "rectangle" /* RECTANGLE */, offsetX: -15, offsetY: 0, rotation: 30 * Math.PI / 180, size: 90 },
        { type: "triangle" /* TRIANGLE */, offsetX: 15, offsetY: 0, rotation: -15 * Math.PI / 180, size: 90 }
      ]
    },
    {
      dropZoneIndex: 1,
      color: "rgba(245, 158, 50, 0.65)",
      opacity: 1,
      shapes: [
        { type: "triangle" /* TRIANGLE */, offsetX: -10, offsetY: 0, rotation: 20 * Math.PI / 180, size: 90 },
        { type: "circle" /* CIRCLE */, offsetX: 10, offsetY: 0, rotation: 0, size: 90 }
      ]
    },
    {
      dropZoneIndex: 2,
      ...
      opacity: 1,
      shapes: [
        { type: "rectangle" /* RECTANGLE */, offsetX: -15, offsetY: 0, rotation: 0, size: 90 },
        { type: "circle" /* CIRCLE */, offsetX: 15, offsetY: 0, rotation: 0, size: 90 }
      ]
    }
  ];
}
...
function drawShape(ctx2, type, x, y, size, color, rotation, alpha) {
  ctx2.save();
  ...
  switch (type) {
    ...
    case "circle" /* CIRCLE */:
      ctx2.fillStyle = color;
      ctx2.beginPath();
      ctx2.arc(0, 0, size / 2, 0, Math.PI * 2);
      ctx2.closePath();
      ctx2.fill();
      ctx2.shadowBlur = 0;
      ctx2.shadowOffsetY = 0;
...
function drawShapeCellophane(targetCtx, type, x, y, size, color, rotation) {
  targetCtx.save();
...
function drawRotationHandle(ctx2, shape) {
  const handleDist = appData.rotationHandleDistance;
  const handleX = shape.x + Math.cos(shape.rotation - Math.PI / 2) * handleDist;
  const handleY = shape.y + Math.sin(shape.rotation - Math.PI / 2) * handleDist;
  ctx2.save();
  ...
  ctx2.beginPath();
  ctx2.moveTo(shape.x, shape.y);
  ctx2.lineTo(handleX, handleY);
  ctx2.stroke();
  ctx2.setLineDash([]);
  const handleGrad = ctx2.createRadialGradient(handleX, handleY, 0, handleX, handleY, appData.rotationHandleRadius);
  handleGrad.addColorStop(0, "#818CF8");
  handleGrad.addColorStop(1, "#6366F1");
  ctx2.fillStyle = handleGrad;
  ctx2.beginPath();
  ctx2.arc(handleX, handleY, appData.rotationHandleRadius, 0, Math.PI * 2);
  ctx2.fill();
  ctx2.strokeStyle = "#FFFFFF";
  ctx2.lineWidth = 2;
  ctx2.stroke();
  ctx2.save();
  ctx2.translate(handleX, handleY);
  ctx2.strokeStyle = "#FFFFFF";
  ctx2.lineWidth = 2;
  ctx2.beginPath();
  ctx2.arc(0, 0, 6, -Math.PI * 0.7, Math.PI * 0.3);
  ctx2.stroke();
  ctx2.beginPath();
  const endAngle = Math.PI * 0.3;
  const endX = Math.cos(endAngle) * 6;
  const endY = Math.sin(endAngle) * 6;
  ctx2.moveTo(endX - 3, endY - 3);
  ctx2.lineTo(endX, endY);
  ctx2.lineTo(endX + 3, endY - 2);
  ctx2.stroke();
  ctx2.restore();
  ctx2.save();
  ctx2.translate(shape.x, shape.y);
  ctx2.rotate(shape.rotation);
  ctx2.strokeStyle = "#6366F1";
  ctx2.lineWidth = 2.5;
  ...
  switch (shape.type) {
    case "rectangle" /* RECTANGLE */:
      ctx2.strokeRect(-half, -half, half * 2, half * 2);
      break;
    case "triangle" /* TRIANGLE */:
      ctx2.beginPath();
      ctx2.moveTo(0, -half);
      ctx2.lineTo(half, half);
      ctx2.lineTo(-half, half);
      ctx2.closePath();
      ctx2.stroke();
      break;
...
function drawDropZones() {
  const labels = [textData.targetLabel1, textData.targetLabel2, textData.targetLabel3];
  for (let i = 0; i < dropZones.length; i++) {
    ...
    ctx.fillText(labels[i] || "", dz.x + dz.width / 2, dz.y + dz.height + 8);
    ctx.restore();
  }
...
function drawClonedShapes() {
  for (const dz of dropZones) {
    ...
    const offCanvas = AppHelper.createUIElement("canvas", "", {}, "");
  ...
  if (selectedShape && currentState === "playing" /* PLAYING */) {
...
function drawGuideCharacter() {
  ...
  ctx.fillText("\u{1F9D1}\u200D\u{1F3EB}", charX, charY);
  ctx.restore();
...
function drawPlayingScreen() {
  ...
  ctx.shadowBlur = 10;
  ctx.shadowOffsetY = 3;
  drawRoundedRect(ctx, 20, 8, appData.canvasWidth - 40, 60, 14);
  ...
  ctx.textAlign = "center";
  ctx.textBaseline = "top";
  ctx.fillText(textData.title, appData.canvasWidth / 2, 17);
  ctx.fillStyle = "#7A8BB5";
...
var targetIntersections, assetListData, nextCloneId, currentState, appData, textData, appCanvas, ctx, uiLayer, clonedShapes, paletteShapes, dropZones, selectedShape, draggingShape, isDraggingFromPalette, isRotating, dragOffsetX, dragOffsetY, rotationStartAngle, shapeStartAngle, currentBubbleText, animFrameId, uiElements, soundsInitialized;
var init_app = __esm({
  "public/20260401183100_f1bbc072-ea26-42d7-a4eb-8de16f4caed3/app.ts"() {
    init_appHelper();
    init_gsap();
