...
function drawRotationHandle(ctx: CanvasRenderingContext2D, shape: ClonedShape): void {
  ...
  const handleGrad = ctx.createRadialGradient(handleX, handleY, 0, handleX, handleY, appData.rotationHandleRadius);
...
function drawInstructionScreen(): void {
  ...
  // Draw example shapes on the right side with harmonious colors
...
function drawPlayingScreen(): void {
  ...
  // Draw header area with gradient accent
  ctx.save();
  ctx.globalAlpha = 1.0;
  ctx.globalCompositeOperation = "source-over";
  ...
  // Thin accent line at top of header
  ctx.save();
...
function playDemoAnimation(): void {
  ...
  // Answer definitions: which shapes go to which box
  // Box 1: Blue Rectangle + Yellow Triangle
  // Box 2: Yellow Triangle + Red Circle
  // Box 3: Blue Rectangle + Red Circle
  ...
  // For each target box, create 2 cloned shapes from palette and record them
  ...
  answerMap.forEach((entry, entryIdx) => {
    ...
    entry.shapes.forEach((shapeDef, shapeIdx) => {
      ...
      // Stagger: each box starts 0.15s apart, each shape within a box 0.1s apart
