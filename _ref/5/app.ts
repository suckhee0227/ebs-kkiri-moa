...
// Target images for drop zones (left, center, right)
let targetImages: HTMLImageElement[] = [];
let rotateIconImg: HTMLImageElement;
...
// HTML guide bubble element reference
let guideBubbleEl: HTMLElement | null = null;
...
function drawShapeIntersectionBlend(shapeA: ClonedShape, shapeB: ClonedShape, dz: DropZone): void {
  ...
  // Clip to shape A, then fill shape B intersection with yellow
  ctx.beginPath();
...
function blendColors(color1: string, color2: string): string {
  // Real color mixing for shape overlaps
...
function checkAllAnswers(): boolean {
  ...
  for (const entry of answerMap) {
    ...
    for (let i = 0; i < entry.shapes.length; i++) {
      ...
      const allowedRots: number[] = (offset as any).allowedRotations || [];
      const anyRot: boolean = (offset as any).anyRotation === true;
      const rotTolerance = 15; // ±15 degrees
...
function drawAllTargetIntersections(): void {
  ...
  // Image-only targets, no code-based fallback
}
...
function drawShape(
  ...
  targetCtx.save();
  // Preserve caller's globalCompositeOperation
...
function drawRotationHandle(targetCtx: CanvasRenderingContext2D, shape: ClonedShape): void {
  ...
  // Selection highlight border (black dashed)
  targetCtx.save();
  ...
  // Calculate icon position based on shape type (in local rotated coords)
  let iconLocalX = 0;
  let iconLocalY = 0;
  ...
  switch (shape.type) {
    case ShapeType.RECTANGLE:
      ...
      iconLocalX = half;    // right edge
      iconLocalY = -half;   // top edge
      break;
    case ShapeType.TRIANGLE: {
      const sideLength = shape.size;
  ...
  // Draw rotate icon at the calculated position (still in rotated coords)
...
function drawClonedShapes(): void {
  ...
  for (const s of nonDraggingShapes) {
    ...
    if (isInDropZone) {
      ...
      ctx.globalCompositeOperation = "source-over";
      if (useHintGlow) {
        drawShapeWithHintGlow(ctx, s.type, s.x, s.y, s.size, s.color, s.rotation, 1.0);
      } else {
        drawShape(ctx, s.type, s.x, s.y, s.size, s.color, s.rotation, 1.0);
      }
    }
    ctx.restore();
  }
  ...
  // Target images already drawn in drawDropZones, no re-draw needed

  // Draw dragging shapes on top
...
function trySnapToTarget(shape: ClonedShape): boolean {
  ...
  for (let i = 0; i < entry.shapes.length; i++) {
    ...
    if (dist < appData.snapThreshold && dist < bestDist) {
      ...
      if (anyRot) {
        // Circle: any rotation OK
        bestDist = dist;
        bestIndex = i;
        bestTargetX = targetX;
        bestTargetY = targetY;
      } else if (allowedRots.length > 0) {
        // Check if current rotation is near any allowed rotation
  ...
  if (bestIndex >= 0) {
    // Find nearest allowed rotation to snap to
    const offset = entry.targetOffsets[bestIndex];
    const allowedRots: number[] = (offset as any).allowedRotations || [];
    const anyRot: boolean = (offset as any).anyRotation === true;
    let bestTargetRot = shape.rotation; // keep current rotation by default
    ...
    gsap.to(shape, {
      ...
      onComplete: () => {
        ...
        // Check if this drop zone is complete
        const dzEntry = appData.answerMap.find((e: any) => e.dropZoneIndex === dzIndex);
        if (dzEntry) {
          const snappedCount = clonedShapes.filter((s) => s.dropZoneIndex === dzIndex && s.snapped).length;
          if (snappedCount >= dzEntry.shapes.length) {
            // This zone complete - mini celebration
            const dz = dropZones[dzIndex];
            if (dz) {
              try {
                confetti({
                  ...
                  spread: 50,
                  origin: {
                    x: (dz.x + dz.width / 2) / appData.canvasWidth,
                    y: (dz.y + dz.height / 2) / appData.canvasHeight,
                  },
...
function checkAutoComplete(): void {
  // Auto-complete disabled - use check button only
}
...
function handleCorrectAnswer(): void {
  ...
  // Stay on current screen (no game over)
  setTimeout(() => {
...
function drawPlayingScreen(): void {
  ...
  // Draw drop zones and palette shapes (no palette area background)
  drawDropZones();
  drawPaletteShapes();
...
function handlePointerDown(e: PointerEvent): void {
  ...
  if (currentState === AppState.TITLE || currentState === AppState.INSTRUCTION) {
    return;
  }
...
function handlePointerMove(e: PointerEvent): void {
  ...
  if (isRotating && selectedShape) {
    // Only the rotation handle updates the angle
    ...
    // Apply 15-degree step snap (same as folder 6)
...
function handlePointerUp(e: PointerEvent): void {
  ...
  if (draggingShape) {
    ...
    if (draggingShape.y > appData.paletteY - appData.shapeSize * 0.5 && draggingShape.dropZoneIndex === -1) {
      ...
      // Check if this shape type belongs in this drop zone
      const dzIdx = draggingShape.dropZoneIndex;
      const dzEntry = appData.answerMap.find((e: any) => e.dropZoneIndex === dzIdx);
      const isValidShape = dzEntry && dzEntry.shapes.some(
        (s: any) => s.type === draggingShape.type && s.color.toUpperCase() === draggingShape.color.toUpperCase()
      );
      ...
      if (!isValidShape) {
        ...
        // Snap judgment ONLY happens here at mouseup
...
function createPlayingUI(): void {
  ...
  // Solid header strip (matching folder 1/2/3/4 style)
  ...
  const sqBack = AppHelper.createUIElement("div", "", {
    position: "absolute", top: "3px", left: "9px", width: "16px", height: "16px",
    border: "2px solid #666", borderRadius: "2px", backgroundColor: "#FFFFFF",
    boxSizing: "border-box", pointerEvents: "none",
  });
  squareIcon.appendChild(sqBack);
  const sqFront = AppHelper.createUIElement("div", "", {
    position: "absolute", top: "9px", left: "3px", width: "16px", height: "16px",
    border: "2px solid #666", borderRadius: "2px", backgroundColor: "#FFFFFF",
    boxSizing: "border-box", pointerEvents: "none",
  });
  ...
  const closeIcon = AppHelper.createUIElement("div", "closeIcon", {
    width: "24px", height: "24px", cursor: "pointer", pointerEvents: "auto",
    display: "flex", alignItems: "center", justifyContent: "center",
    fontSize: "20px", fontWeight: "bold", color: "#666", boxSizing: "border-box",
  }, "✕");
...
function handleCheckAnswer(): void {
  ...
  // First check if user already placed shapes correctly
  ...
  // Otherwise, animate shapes to correct positions
...
// Function to build the HTML character + speech bubble overlay (folder 1 style)
function buildCharacterUI(): void {
  ...
  // Speech bubble wrapper (right of character)
...
async function initApp(): Promise<void> {
  ...
  // Load target images for drop zones
